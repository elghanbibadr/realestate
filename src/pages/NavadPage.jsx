...
const NavadPage = () => {
  return (
    <div>
      ...
      {/* BODY */}
      <div className="mt-20 ">
        <h2 className="text-center dinNextLtRegular font-semibold text-darkGreen text-[30px]">
          الدخول على النظام
        </h2>
        {/* navad surface card*/}
        <div className="w-[80%] mx-auto  bg-[#F1F1F1] ">
          <div className="bg-darkGreen rounded-md mt-6 p-4">
            <h4 className="text-white dinNextLtRegular font-semibold text-[20px] text-center">
              تطبيق نفاد
            </h4>
          </div>
          <div className="w-[80%] grid grid-cols-2 mx-auto mt-10 p-6 pb-10 bg-white rounded-md">
            <div className="">
              ...
              <p className="text-textColor dinNextLtRegular font-semibold text-center  text-[15px]">
                {" "}
                الرجاء ادخال رقم بطاقة الأحوال/الاقامة ثم <br />
                اضغط دخول
              </p>
            </div>
            <div className="text-right">
              <p className="text-textColor font-semibold  text-[15px]">
                {" "}
                رقم بطاقة الأحوال/الاقامة{" "}
              </p>
              <input
                type="text"
                placeholder="ادخل رقم بطاقة الأحوال/الاقامة"
                className="input input-bordered input-accent border-darkGreen placeholder:opacity-30 w-full my-3 text-right mb-6"
              />
              <Link to="/properties">
                <button className="btn bg-darkGreen text-white w-full btn-accent hover:bg-darkGreen">
                  {" "}
                  تسجيل الدخول{" "}
                </button>
              </Link>
              <p className="text-textColor dinNextLtRegular font-semibold  text-[15px] text-center mt-6">
                لتحميل تطبيق نفاد
              </p>
              <div className="grid grid-cols-3 gap-4   mt-4">
                <img
                  className=" h-[80%] object-fill  w-full"
                  src={availableonappgallery}
      ...
      <div className="rightDir flex justify-between p-4  bg-[#F1F1F1] mt-20 py-8">
        <div className="flex">
          ...
          <div>
            <p className="text-black dinNextLtRegular font-semibold   text-[15px]">
              تطوير وتشغيل
            </p>
            <h3 className="dinNextLtRegular font-semibold text-[#5E6C79] text-[21px]">
              مركز المعلومات الوطني
            </h3>
            <p className="text-black dinNextLtRegular font-semibold   text-[15px]">
              النفاد الوطني الموحد جميع الحقوق محفوظة 2023{" "}
            </p>
          </div>
        </div>
